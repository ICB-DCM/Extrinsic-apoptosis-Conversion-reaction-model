...
function varargout = logLMEMOIR(varargin)
    ...
    for s = reshape(batchIndices, [1, numel(batchIndices)])
        ...
        if isfield(Data{s},'SCTL')
            ...
            if options.nderiv >= 1
                dlogLdxi = dlogLdxi + transpose(scaling*sum(dlogL_scdxi, 1));
                if options.nderiv >= 2
...
%             
        ...
        if isfield(Data{s},'SCSH')
            ...
            % Summation
            logL = logL + options.scaleSCSH * logL_m + options.scaleSCSH * logL_C;
            if options.nderiv >= 1
                dlogLdxi = dlogLdxi + options.scaleSCSH * dlogL_mdxi + options.scaleSCSH * dlogL_Cdxi;
                if options.nderiv >= 2
        ...
        if isfield(Data{s},'PA')
            ...
            % Summation
            logL = logL + options.scalePA * logL_m;
            if options.nderiv >= 1
                dlogLdxi = dlogLdxi + options.scalePA * dlogL_mdxi;
                if options.nderiv >= 2
