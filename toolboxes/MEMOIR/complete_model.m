...
switch(Model.type_D)
    case {'matrix-logarithm', 'Givens-parametrization', 'Lie-generators', 'Cholesky', 'spherical'}
...
if(~loadold)
    ...
    switch(Model.type_D)
        ...
        case {'matrix-logarithm', 'cholesky-parametrization', 'spherical'}
            l = 1;
            for j = 1:n_b
                for k = 1:j
    ...
    for s = 1:length(S)
        ...
        for j = 1:size(Model.exp{s}.sym.sigma_noise,1)
            for k = 1:size(Model.exp{s}.sym.sigma_noise,2)
                Model.exp{s}.sym.dsigma_noisedphi(j,k,:) = permute(jacobian(Model.exp{s}.sym.sigma_noise(j,k),phi), [1, 3, 2]);
            end
        end
