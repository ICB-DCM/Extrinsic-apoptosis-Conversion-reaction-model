...
% along with the other parameters, or compute it from replicates.
    ...
    if isfield(Model.exp{s}, 'SPapprox')
        optionsSim.approx = Model.exp{s}.SPapprox;
    else
        optionsSim.approx = 'sp';
    end
    if strcmp(optionsSim.approx, 'cmd')
        optionsSim.n_samples = Model.exp{s}.n_samples;
    end
    ...
    switch Model.exp{s}.estimate_sigma
        case 0 
            ...
            switch Model.exp{s}.noise_model
                case 'normal'
                    ...
                    if (options.estimate_sigma == 1)
                        ...
                        dresSigma_mdxi = ((1 ./ Sigma_m(:)) * ones(1,length(xi))) .* reshape(repmat(dSigma_mdxi, [1, size(Data{s}.SCSH.m, 2), 1]), numel(Data{s}.SCSH.m), length(xi));
                        ...
                        dresSigma_mdxi2 = ((sqrt(3) * res ./ (Sigma_m(:).^2)) * ones(1,length(xi))) .* reshape(repmat(dSigma_mdxi, [1, size(Data{s}.SCSH.m, 2), 1]), numel(Data{s}.SCSH.m), length(xi));
                        dresSigma_mdxi2(nan_ind,:) = 0;
                        ddlogL_mdxi2 = ddlogL_mdxi2 - transpose(dresSigma_mdxi2) * dresSigma_mdxi2;

                        if ~strcmp(optionsSim.approx, 'pa only')
                            % Term 1 coming from s_sigma' * s_sigma
                            dresSigma_Cdxi = ((1 ./ Sigma_C(:)) * ones(1,length(xi))) .* reshape(repmat(dSigma_Cdxi, [size(Data{s}.SCSH.C, 2), size(Data{s}.SCSH.C, 3), 1]), numel(Data{s}.SCSH.C), length(xi));
                            dresSigma_Cdxi(nan_ind,:) = 0;
                            ddlogL_Cdxi2 = ddlogL_Cdxi2 + transpose(dresSigma_Cdxi) * dresSigma_Cdxi;

                            % Term 2 coming from s_sigma' * s_sigma
                            res = Cy(:) - Data{s}.SCSH.C(:);
                            dresSigma_Cdxi2 = ((sqrt(3) * res ./ (Sigma_C(:).^2)) * ones(1,length(xi))) .* reshape(repmat(dSigma_Cdxi, [size(Data{s}.SCSH.C, 2), size(Data{s}.SCSH.C, 3), 1]), numel(Data{s}.SCSH.C), length(xi));
