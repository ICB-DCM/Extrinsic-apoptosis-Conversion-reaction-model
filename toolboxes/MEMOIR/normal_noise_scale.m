...
function J_D = normal_noise(Y,Ym,Sigma,ind,nderiv)
    ...
    if nderiv >= 0
        % J_D
        % consider number of data points in likelihood
        data_point_num = sum(~isnan(Ym));
        rep_data_point_num = repmat(data_point_num, [size(Sigma, 1), 1]);
        J_D.val = nansum((0.5*((Y - Ym(:))./Sigma(:)).^2 + 0.5*log(2*pi*Sigma(:).^2)) .* rep_data_point_num(:));
%         J_D.val = nansum(0.5*((Y - Ym(:))./Sigma(:)).^2 + 0.5*log(2*pi*Sigma(:).^2));
        if nderiv >= 1
            % dJ_DdY
            J_D.dY = transpose((Y - Ym(:))./(Sigma(:).^2) .* rep_data_point_num(:));
            J_D.dY(isnan(J_D.dY)) = 0;
            % dJ_DdSigma
            J_D.dSigma = transpose(- ((((Y - Ym(:)).^2)./(Sigma(:).^3)) + 1./Sigma(:)) .* rep_data_point_num(:));
            J_D.dSigma(isnan(J_D.dSigma)) = 0;
            if nderiv >= 2
                %ddJ_DdYdY
                J_D.dYdY = diag((1./(Sigma(:).^2)) .* rep_data_point_num(:));
                J_D.dYdY(isnan(J_D.dYdY)) = 0;
                %ddJ_DdYdSigma
                J_D.dYdSigma = diag(-2*((Y - Ym(:))./(Sigma(:).^3)) .* rep_data_point_num(:));
                J_D.dYdSigma(isnan(J_D.dYdSigma)) = 0;
                %ddJ_DdSigmadSigma
                J_D.dSigmadSigma = diag((3*(((Y - Ym(:)).^2)./(Sigma(:).^4)) - 1./(Sigma(:).^2)) .* rep_data_point_num(:));
