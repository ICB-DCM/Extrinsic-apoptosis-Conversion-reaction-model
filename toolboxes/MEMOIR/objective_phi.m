...
% if sigma_noise is different for each cell
if size(Sigma_noise.val, 3) == size(data.SCTL.Y, 3)
    Sigma_noise.val = Sigma_noise.val(:, :, i);
end
...
% if sigma_noise is function of phi
if size(Y.dphi, 2) < size(phi, 1)
    Y.dphi = [Y.dphi, zeros(size(Y.dphi, 1), size(phi, 1) - size(Y.dphi, 2))];
end
