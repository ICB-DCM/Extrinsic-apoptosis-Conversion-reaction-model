function [SP,logL_m,dlogL_mdxi,ddlogL_mdxi2] = logL_PA(xi, Model, Data, s, options)
    ...
    if isfield(Model.exp{s}, 'uncorr')
        optionsSim.uncorr = Model.exp{s}.uncorr;
    end
        ...
        if (nderiv >= 2)
            switch Model.exp{s}.noise_model
                case 'normal'
                    ...
                    dresSigmadxi = ((1 ./ Sigma(:)) * ones(1,length(xi))) .* reshape(repmat(dSigmadxi, [1, size(Data{s}.PA.m, 2), 1]), numel(Data{s}.PA.m), length(xi));
                    dresSigmadxi(nan_ind,:) = 0;
                    ddlogL_mdxi2 = ddlogL_mdxi2 + transpose(dresSigmadxi) * dresSigmadxi;
                    
                    % Term 2 coming from s_sigma' * s_sigma
                    res = my(:) - Data{s}.PA.m(:);
                    dresSigmadxi2 = ((sqrt(3) * res ./ (Sigma(:).^2)) * ones(1,length(xi))) .* reshape(repmat(dSigmadxi, [1,size(Data{s}.PA.m, 2),1]), numel(Data{s}.PA.m), length(xi));
