function [model] = model_C8S_H_PAt0_syms()
    ...
    p = [KDR KDL kon_FADD koff_FADD kon_p55 kpd_cis ked_tr_p55 ked_tr_p43 kp18i kBID kprobes_s_C8...
        CD95_0H FADD_0 p55_0 BID_0 PrERF1_0H PrNESF2_0H volume_0];
    % fixed parameters
    k = [CD95L];
    ...
    %% INITIAL CONDITIONS
    x0 = sym(zeros(15,1));
    x0(1) = KDL^2*CD95L*CD95_0H^3/((CD95L+KDL)*(KDL^2*CD95_0H^2+KDR*CD95L^2+2*KDR*KDL*CD95L+KDR*KDL^2));
    x0(2) = FADD_0;
    x0(4) = p55_0;
    x0(10) = BID_0;
    x0(12) = PrERF1_0H;
    ...
    y(1) = volume_0*CD95_0H;%CD95
    y(2) = volume_0*(FADD+DISC+DISCp55+p43+p30);%FADD
    y(3) = volume_0*(p55+DISCp55+p43+p30+p18+p18i);%p55
    y(4) = volume_0*(BID+tBID);%BID
    y(5) = volume_0*(PrERF1+PrER);%PrERmGFP
    y(6) = volume_0*(PrNESF2+PrNES);%PrNESmCh
    y(7) = volume_0;
    %% SYSTEM STRUCT
    model.sym.x = x;
    model.sym.u = u;
